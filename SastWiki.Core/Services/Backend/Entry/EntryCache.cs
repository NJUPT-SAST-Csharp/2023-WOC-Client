...
{
    private readonly TimeSpan _expireTime = new(0, 10, 0);
    private Dictionary<string, string> _cahceFileID = [];
    private readonly object _entryMetadataListlock = new();
    ...
    public async Task InitializeAsync()
    {
        ...
        try
        {
            Task<Dictionary<string, string>?> task = _settings.GetItem<Dictionary<string, string>>(
                "EntryCacheList"
            );
    ...
    public async Task AddAsync(string key, EntryDto value)
    {
        ...
        try
        {
            if (_cahceFileID.ContainsKey(key) && await _storage.ContainsAsync(_cahceFileID[key])) // 如果已经有了对应的缓存文件……
            {
                await _storage.LockCacheFile(_cahceFileID[key]);
                FileStream fileStream = await _storage.GetCacheFileStreamAsync(_cahceFileID[key]);
                using FileStream cacheFileStream = fileStream;
            ...
            {
                string ID = await _storage.CreateCacheFileAsync(_expireTime); // 否则创建一个新的缓存文件

                using FileStream cacheFileStream = await _storage.GetCacheFileStreamAsync(ID);
                using var writer = new StreamWriter(cacheFileStream);
    ...
    public async Task<EntryDto?> GetAsync(string key)
    {
        ...
        if (await ContainsAsync(key))
        {
            ...
            try
            {
                using FileStream fs = await _storage.GetCacheFileStreamAsync(_cahceFileID[key]);
                string data = await new StreamReader(fs).ReadToEndAsync();
