...
) : ObservableObject, INavigationAware
{
    ...
    private readonly TaskCompletionSource<bool> _ensureWebviewInitialized = new();

    private async Task SetRequestEventHandler()
    {
        _ = await _ensureWebviewInitialized.Task;
        if (_webview != null)
        {
    ...
    public ICommand EditCommand =>
        new RelayCommand(
            () =>
            {
                ...
                _ = navigationService.NavigateTo(App.GetService<EditPage>(), (int)CurrentEntry.Id!);
            },
            () => IsLoaded
        );
    ...
    )
    {
        if (e.IsUserInitiated) // 判断是否为用户点击，以排除掉图片请求
        {
            ...
            if (Uri.TryCreate(e.Uri, UriKind.Absolute, out Uri? result))
            {
                ...
                {
                    // 从默认浏览器打开链接
                    _ = System.Diagnostics.Process.Start("explorer.exe", e.Uri);
                    e.Cancel = true;
                }
            }
        }
    ...
    private async void LoadMarkdownDoc(object? sender, PropertyChangedEventArgs e)
    {
        if (e.PropertyName == nameof(Markdown_text))
        {
            // string css_text;
            markdownProcessor.Output(Markdown_text, out string html_text, out _);
    ...
    private async Task LoadPage(int id)
    {
        ...
        try
        {
            EntryDto entry = await entryProvider.GetEntryByIdAsync(id);
